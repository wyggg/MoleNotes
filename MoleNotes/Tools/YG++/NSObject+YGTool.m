...
//从本地取出model
+ (instancetype)modelDataInFilePath:(NSString *)filePath{
    id modelData = [NSKeyedUnarchiver unarchiveObjectWithFile:filePath];
    id model = [self parsingObj:modelData];
    return model;
}

+ (instancetype)modelDataInFileName:(NSString *)fileName{
   NSMutableString *docPath =  NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject.mutableCopy;
    [docPath appendString:fileName];
    return [self modelDataInFilePath:docPath];
}

///储存model
- (BOOL)saveValuesInFilePath:(NSString *)filePath{
    id objData = [self objValues];
    return [NSKeyedArchiver archiveRootObject:objData toFile:filePath];
}

- (BOOL)saveValuesInFileName:(NSString *)fileName{
    NSMutableString *docPath =  NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES).lastObject.mutableCopy;
    [docPath appendString:fileName];
    return [self saveValuesInFilePath:docPath];
}
