...
///根据宽度计算字符串高度
- (CGFloat)textHeightWithW:(CGFloat)strWidth fontSize:(CGFloat)fontSize{
    return [self textSize:CGSizeMake(strWidth, __FLT_MIN__) fontSize:fontSize].width;
}
///根据高度计算字符串宽度
- (CGFloat)textWithWithH:(CGFloat)strHeight fontSize:(CGFloat)fontSize{
    return [self textSize:CGSizeMake(__FLT_MIN__, strHeight) fontSize:fontSize].height;
}
...
- hasColorGlyphsInRange:(id)a attributes:(id)b{
    NSLog(@"警告 NSNull调用 hasColorGlyphsInRange 方法");
    return nil;
}
